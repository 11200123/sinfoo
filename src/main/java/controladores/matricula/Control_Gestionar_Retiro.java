 ...
 * @author eduardo
 */
@ManagedBean(name = "gestionaretiro")
@SessionScoped
public class Control_Gestionar_Retiro implements Serializable {
    ...
    public Control_Gestionar_Retiro() {
        panelActivo = 0;
    ...
    private void validarBusqueda() {
        if (matriculaSeleccionada == null) {
            ...
            if(matriculaSeleccionada.getEstadoMatricula()==1){
                RequestContext.getCurrentInstance().execute("PF('debe_eliminar').show();");
                panelActivo = 2;
            }
            else{
                if(matriculaSeleccionada.getAlumno().getEstadoActivo()==0){
                    RequestContext.getCurrentInstance().execute("PF('ya_retirado').show();");
                    panelActivo = 2;
                }
    ...
    void validarMotivoRetiro(){
        if(matriculaSeleccionada.getAlumno().getMotivoRetiro().equals("")){
    ...
    private void retirarAlumno() {
        matriculaSeleccionada.getAlumno().setEstadoActivo(0);
